...
- **Sem viewer de PDF ainda** — cards `PDF` na Home não abrem nada (não estão em `OPENABLE_TYPES`). Isso era "não priorizado por falta de conteúdo" até a migração de 42 PDFs (Eixo Tecnológico → Coleção Mão na massa, ver seção de migração acima); agora é um gap de verdade. Detalhe pra quando for implementar: o "PDF" do legado é na real um link do **FlipHTML5** (visualizador de flipbook de terceiros, não um arquivo `.pdf` puro) — dá pra simplesmente embedar o link deles num iframe (mais simples) em vez de reimplementar viewer com `pdf.js`/`react-pageflip` como o plano original sugeria, já que o link cru já é a experiência de "livro" pronta.
